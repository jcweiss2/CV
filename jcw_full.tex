%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Medium Length Professional CV
% LaTeX Template
% Version 2.0 (8/5/13)
%
% This template has been downloaded from:
% http://www.LaTeXTemplates.com
%
% Original author:
% Trey Hunner (http://www.treyhunner.com/)
%
% Important note:
% This template requires the resume.cls file to be in the same directory as the
% .tex file. The resume.cls file provides the resume style used for structuring the
% document.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%----------------------------------------------------------------------------------------
%	PACKAGES AND OTHER DOCUMENT CONFIGURATIONS
%----------------------------------------------------------------------------------------

\documentclass{resume2} % Use the custom resume.cls style

\usepackage[left=0.75in,top=0.6in,right=0.75in,bottom=0.6in]{geometry} % Document margins
\usepackage{hyperref}

\name{Jeremy C. Weiss} % Your name
\address{Heinz College \\ Carnegie Mellon University} % Your address
\address{5000 Forbes Ave, Hamburg Hall \\ Pittsburgh, PA 15213} % Your secondary addess (optional)
\address{\href{jeremyweiss@cmu.edu}{jeremyweiss@cmu.edu} \\ (412)~$\cdot$~268~$\cdot$~5726} % Your phone number and email

\begin{document}

%----------------------------------------------------------------------------------------
%	EDUCATION SECTION
%----------------------------------------------------------------------------------------

\begin{rSection}{Education}

\begin{tabbing}
M.D.  \hspace*{2.32 em}\= \textbf{University of Wisconsin--Madison} \hspace*{5em} \= \hspace*{5em} \= \` 2007 -- 2009, 2014 -- 2016\\
\> School of Medicine and Public Health \\
Ph.D. \hspace*{2 em}\= \textbf{University of Wisconsin--Madison} \hspace*{5em} \= \hspace*{10em} \= \` 2009 -- 2014\\
\> Department of Computer Sciences \\
%
B.A./B.S. \hspace*{2 em}\> \textbf{University of Pennsylvania} \> \> \` 2003 -- 2007 \\
\> Department of Mathematics, Department of Biochemistry \\
%
H.S.\hspace*{2 em}\> \textbf{The Lakeside School} \> \> \` 1999 -- 2003 \\
\end{tabbing}
\end{rSection}

%----------------------------------------------------------------------------------------
%	RESEARCH EXPERIENCE SECTION
%----------------------------------------------------------------------------------------
\begin{rSection}{Research Experience}
\input{research-experience}
\end{rSection}

%----------------------------------------------------------------------------------------
%	TEACHING EXPERIENCE SECTION
%----------------------------------------------------------------------------------------
\begin{rSection}{Teaching Experience}
\input{teaching-experience}
\end{rSection}

%----------------------------------------------------------------------------------------
%	PUBLICATIONS	
%----------------------------------------------------------------------------------------
%\clearpage
\begin{rSection}{Publications}
\input{publications}
\end{rSection}


%----------------------------------------------------------------------------------------
%	SELECTED PRESENTATIONS
%----------------------------------------------------------------------------------------
\begin{rSection}{Presentations and Abstracts}
\input{presentations}
\end{rSection}

%----------------------------------------------------------------------------------------
%	HONORS AND AWARDS
%----------------------------------------------------------------------------------------
\vspace*{0.3 em}
%\clearpage
\begin{rSection}{Honors And Awards}
\input{honors-and-awards}
\end{rSection}

%----------------------------------------------------------------------------------------
%	SERVICE
%----------------------------------------------------------------------------------------
%\vspace*{1 em}
\begin{rSection}{Professional Service}
\input{professional-service}
\end{rSection}

%----------------------------------------------------------------------------------------
%	COMPUTER SKILLS
%----------------------------------------------------------------------------------------
%% %\clearpage
%% %\vspace*{1 em}
%% \begin{rSection}{Computer Skills}
%% \begin{tabular}{ @{} >{\bfseries}l @{\hspace{6ex}} l }
%% Languages & R, Python, PyTorch, C++, Java, Scala/Spark, SQL, JavaScript \\
%% %Versioning and Testing & git, svn, pdb, valgrind \\
%% Tools & \LaTeX, Markdown, RMarkdown, shell, bash, Emacs, eclipse, git\\
%% \end{tabular}

%% \end{rSection}

%----------------------------------------------------------------------------------------
%	EXAMPLE SECTION
%----------------------------------------------------------------------------------------

\begin{rSection}{Media}

\input{media}

\end{rSection}

%----------------------------------------------------------------------------------------

\end{document}
	
